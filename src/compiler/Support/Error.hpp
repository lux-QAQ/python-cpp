...
{
    switch (kind) {
    case ErrorKind::BitcodeLoadFailed: return "BitcodeLoadFailed";
    case ErrorKind::AnnotationParseFailed: return "AnnotationParseFailed";
    case ErrorKind::RuntimeFunctionNotFound: return "RuntimeFunctionNotFound";
    case ErrorKind::RuntimeFunctionSignatureMismatch: return "RuntimeFunctionSignatureMismatch";
    case ErrorKind::CodegenUnsupportedNode: return "CodegenUnsupportedNode";
    case ErrorKind::CodegenTypeError: return "CodegenTypeError";
    case ErrorKind::CodegenInternalError: return "CodegenInternalError";
    case ErrorKind::ModuleNotFound: return "ModuleNotFound";
    case ErrorKind::SymbolConflict: return "SymbolConflict";
    case ErrorKind::LinkError: return "LinkError";
    case ErrorKind::VerificationFailed: return "VerificationFailed";
    case ErrorKind::OptimizationFailed: return "OptimizationFailed";
    case ErrorKind::IOError: return "IOError";
    case ErrorKind::InvalidArgument: return "InvalidArgument";
    case ErrorKind::NotImplemented: return "NotImplemented";
...
template<typename... Args>
[[nodiscard]] std::unexpected<CompilerError> make_error_impl(
    std::source_location loc,
    ErrorKind kind,
...
[[nodiscard]] inline std::unexpected<CompilerError> make_error_impl(
    std::source_location loc,
    ErrorKind kind,
    std::string msg)
{
...
#define INTERNAL_ERROR(...) \
    MAKE_ERROR(::pylang::ErrorKind::InternalError, __VA_ARGS__)

#define NOT_IMPLEMENTED(...) \
    MAKE_ERROR(::pylang::ErrorKind::NotImplemented, __VA_ARGS__)
...
/// and_then 管道适配器
template<typename F>
struct AndThen { F func; };

template<typename F>
[[nodiscard]] constexpr auto and_then(F &&f) { return AndThen<std::decay_t<F>>{std::forward<F>(f)}; }

template<typename T, typename F>
[[nodiscard]] auto operator|(Result<T> &&r, AndThen<F> &&a)
{
...
/// transform 管道适配器
template<typename F>
struct Transform { F func; };

template<typename F>
[[nodiscard]] constexpr auto transform(F &&f) { return Transform<std::decay_t<F>>{std::forward<F>(f)}; }

template<typename T, typename F>
[[nodiscard]] auto operator|(Result<T> &&r, Transform<F> &&t)
{
...
/// or_else 管道适配器
template<typename F>
struct OrElse { F func; };

template<typename F>
[[nodiscard]] constexpr auto or_else(F &&f) { return OrElse<std::decay_t<F>>{std::forward<F>(f)}; }

template<typename T, typename F>
[[nodiscard]] auto operator|(Result<T> &&r, OrElse<F> &&o)
{
...
template<typename T>
[[nodiscard]] T unwrap_or_die(Result<T> &&result, std::string_view msg = "")
{
...
template<typename T>
[[nodiscard]] T unwrap_or(Result<T> &&result, T default_value)
{
    if (result) { return *std::move(result); }
...
template<typename T>
[[nodiscard]] Result<T> ok_or(std::optional<T> &&opt, ErrorKind kind, std::string msg,
...
template<typename T, typename F>
    requires std::invocable<F>
          && std::same_as<std::invoke_result_t<F>, CompilerError>
...
#define PYLANG_ASSERT(cond, ...)                                             \
    do {                                                                     \
        if (!(cond)) [[unlikely]] {                                          \
            ...
            auto _err = ::pylang::CompilerError(                             \
                ::pylang::ErrorKind::InternalError,                          \
...
#define PYLANG_UNREACHABLE(...)                                              \
    do {                                                                     \
        ...
        __VA_OPT__(                                                          \
            _pylang_unreach_msg += ": ";                                      \
            _pylang_unreach_msg += std::format(__VA_ARGS__);                  \
        )                                                                    \
        auto _err = ::pylang::CompilerError(                                 \
            ::pylang::ErrorKind::InternalError,                              \
...
public:
    ...
    template<typename T>
    std::optional<T> try_collect(Result<T> &&result)
    {
        if (result) { return *std::move(result); }
