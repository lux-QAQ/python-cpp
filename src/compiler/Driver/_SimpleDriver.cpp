...
Result<std::unique_ptr<llvm::TargetMachine>> SimpleDriver::create_host_target_machine(
    int opt_level)
{
    ...
    auto *tm = target->createTargetMachine(triple_str,
        "generic",
        "",
        target_opts,
        llvm::Reloc::PIC_,
    ...
    if (!tm) {
        return MAKE_ERROR(ErrorKind::InternalError, "Failed to create TargetMachine");
    }
...
Result<SimpleDriver> SimpleDriver::create(Options opts, llvm::LLVMContext &ctx)
{
    ...
    log::compiler()->info("SimpleDriver created (runtime: {}, opt: O{})",
        opts.runtime_bc_path.string(),
        opts.opt_level);

    return SimpleDriver(std::move(opts), ctx, std::move(loader), std::move(linker), std::move(tm));
}
...
Result<std::unique_ptr<llvm::Module>> SimpleDriver::stage_codegen(
    ast::Module &module, std::string_view name)
{
    PYLANG_TIMER("stage_codegen");
...
{
    PYLANG_TIMER("stage_link_runtime");
    log::compiler()->info("Stage 2: Linking runtime into '{}'", user_mod->getName().str());

    return m_linker.link_into(user_mod.get())
        .transform([&user_mod]() { return std::move(user_mod); });
}

VoidResult SimpleDriver::stage_optimize(llvm::Module &module)
{
    PYLANG_TIMER("stage_optimize");
    log::compiler()->info("Stage 3: Optimizing '{}' at O{}", module.getName().str(), m_opts.opt_level);
...
Result<std::filesystem::path> SimpleDriver::stage_to_object(
    llvm::Module &module, const std::filesystem::path &output)
{
    PYLANG_TIMER("stage_to_object");
    ...
    if (m_target_machine->addPassesToEmitFile(
        ...
        return MAKE_ERROR(
            ErrorKind::InternalError, "TargetMachine cannot emit object file");
    }
...
Result<std::filesystem::path> SimpleDriver::stage_to_bitcode(
    llvm::Module &module, const std::filesystem::path &output)
{
    std::error_code ec;
    llvm::raw_fd_ostream dest(output.string(), ec, llvm::sys::fs::OF_None);
...
{
    PYLANG_TIMER("stage_link_executable");

    auto exe_path = m_opts.output_path.empty()
        ? obj_path.parent_path() / obj_path.stem()
        : m_opts.output_path;
    ...
    // 添加用户指定的额外链接参数
    for (const auto &flag : m_opts.extra_link_flags) {
        cmd += " " + flag;
    }
    ...
    if (ret != 0) {
        return MAKE_ERROR(ErrorKind::LinkError,
            "System linker failed (exit code {}): {}", ret, cmd);
    }
...
std::filesystem::path SimpleDriver::infer_output_path(
    std::string_view module_name, OutputKind kind) const
{
    ...
    switch (kind) {
    case OutputKind::IRText: return base.replace_extension(".ll");
    case OutputKind::Bitcode: return base.replace_extension(".bc");
    case OutputKind::ObjectFile: return base.replace_extension(".o");
    case OutputKind::Executable: return base;
    }
    return base;
}
...
Result<std::string> SimpleDriver::emit_unoptimized_ir(
...
Result<std::string> SimpleDriver::compile_to_ir(
    ast::Module &module, std::string_view module_name)
{
    // AST → codegen → link runtime → optimize → IR text
    return stage_codegen(module, module_name)
        .and_then([this](auto mod) {
            return stage_link_runtime(std::move(mod));
        })
...
Result<std::filesystem::path> SimpleDriver::compile_module(
    ast::Module &module, std::string_view module_name)
{
    ...
    return stage_codegen(module, module_name)
        ...
        .and_then([this, &output](auto mod) -> Result<std::filesystem::path> {
            ...
            case OutputKind::Executable: {
                ...
                return stage_to_object(*mod, obj)
                    .and_then([this](auto obj_path) {
                        return stage_link_executable(obj_path);
                    });
            }
            }
