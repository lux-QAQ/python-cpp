...
PyResult<PyObject *> PyType::call_raw(std::span<Value> args, PyDict *kwargs)
{
    spdlog::trace("[call_raw] PyType::call_raw for {}", name());

    // 1. 调用 __new__
    // 对于大多数类，__new__ 不处理参数，可以直接传空元组以节省开销
    // 但为了 100% 语义正确，如果有参数，我们还是得打包一个临时的给 __new__
    PyTuple* new_args = nullptr;
    if (!args.empty()) {
        auto res = PyTuple::create(args);
        if (res.is_err()) return Err(res.unwrap_err());
        new_args = res.unwrap();
    }
    
    auto obj_res = new_(new_args, kwargs);
    if (obj_res.is_err()) return obj_res;
    auto *obj = obj_res.unwrap();
    
    // 2. 遵循 Python 语义：只有当返回的对象是当前类或其子类的实例时，才调用 __init__
    if (obj->type()->issubclass(this)) {
        // [关键修复]：必须调用 init_raw 并传入原始 args span
        auto init_res = obj->init_raw(args, kwargs);
        if (init_res.is_err()) return Err(init_res.unwrap_err());
        
        if (init_res.unwrap() < 0) {
            return Err(runtime_error("__init__ returned negative value"));
        }
    }
