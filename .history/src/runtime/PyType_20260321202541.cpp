...
PyResult<PyObject *> PyType::call_raw(std::span<Value> args, PyDict *kwargs)
{
    ...
    if (obj_res.is_err()) {
        spdlog::error("[Type::call_raw] __new__ failed for {}", name());
        return obj_res;
    }
    auto *obj = obj_res.unwrap();
    ...
    if (obj->type()->issubclass(this)) {
        spdlog::debug("[Type::call_raw] Dispatching to init_raw for obj: {:p}, argc: {}", (void*)obj, args.size());
        auto init_res = obj->init_raw(args, kwargs);
        if (init_res.is_err()) {
            spdlog::error("[Type::call_raw] init_raw failed for obj: {:p}", (void*)obj);
            return Err(init_res.unwrap_err());
        }
        spdlog::debug("[Type::call_raw] init_raw returned: {}", init_res.unwrap());
    } else {
        spdlog::warn("[Type::call_raw] Skip __init__: obj type {} is not subclass of {}", obj->type()->name(), name());
    }
...
{
    auto *name_str = as<PyString>(name);
    std::string n = name_str ? name_str->value() : "unknown";
    
    auto mro_res = mro_internal();
    if (mro_res.is_err()) return std::nullopt;

    spdlog::trace("[Type::lookup] Searching '{}' in MRO of '{}'", n, name());

    for (const auto &t_ : mro_res.unwrap()->elements()) {
        auto *t = as<PyType>(py::ensure_box(t_));
        if (!t) continue;
        
        if (t->m_attributes) {
            auto it = t->m_attributes->map().find(py::Value(name));
            if (it != t->m_attributes->map().end()) {
                auto val = py::PyObject::from(it->second);
                spdlog::trace("[Type::lookup] FOUND '{}' in '{}'", n, t->name());
                return val;
            }
        }
    }
